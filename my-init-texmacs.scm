...
(kbd-map
  (:mode in-math?)
  ;; Using 'space space' to go out, from right, of the
  ;; tmp structure.
  ;; This can be established only in math-mode.
  ("space space" (structured-exit-right))
  ;; Following vim's keybinding, use "l": 
  ("l l" (structured-exit-right))
  ("space l" (structured-exit-right))
  ("l space" (structured-exit-right))
  ;; Also from the left.
  ;; This can be established only in math-mode.
  ;; Following vim's keybinding, use "h": 
  ("h h" (structured-exit-left))
  ("space h" (structured-exit-left))
  ("h space" (structured-exit-left))
  )
